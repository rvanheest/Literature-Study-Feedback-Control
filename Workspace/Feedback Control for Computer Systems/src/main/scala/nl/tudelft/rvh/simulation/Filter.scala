...
class Identity(value: Double = 0) extends Component {
...
class FixedFilter(n: Int, data: List[Double] = List()) extends Component {
...
class RecursiveFilter(alpha: Double, y: Double = 0) extends Component {
...
class Limiter(lo: Double, hi: Double, res: Double = 0) extends Component{
...
class Integrator(data: Double = 0)(implicit DT: Double) extends Component {
	def update(u: Double): Integrator = new Integrator(data + u)
	
	def action = DT * data
}

class Discretizer(binwidth: Double, res: Double = 0) extends Component {
...
class Hysteresis(threshold: Double, prev: Double = 0, res: Double = 0) extends Component {
